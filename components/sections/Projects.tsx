  ...
  const projects = [
    {
      ...
      desc: "An intelligent Serverless RAG chatbot integrated directly into this portfolio. Built with Gemini embeddings and Groq Llama 3.",
      img: "/p7.png", // Reusing Panda AI image for now
        ...
        {projects.map((p, i) => {
          const content = (
            <motion.div
              ...
              className="group relative flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-8 bg-neutral-900/40 backdrop-blur-md rounded-[24px] md:rounded-[28px] p-5 sm:p-6 md:p-8 hover:bg-neutral-800/60 border border-white/5 hover:border-white/10 transition-all duration-300 overflow-hidden cursor-pointer w-full"
            >
              {/* TOP RIGHT ICON */}
              {p.internal ? (
                <ArrowRight 
                  size={22} 
                  className="absolute top-5 right-5 md:top-6 md:right-6 text-[#ff6b00] opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 ease-out" 
                />
              ) : (
                <ExternalLink 
                  size={22} 
              ...
              {/* TEXT */}
              <div className="sm:pr-12 mt-1 sm:mt-0 relative z-10 flex-1">
          ...
          return p.internal ? (
            <Link href={p.link} key={i} className="block w-full">
              {content}
            </Link>
          ) : (
            <a href={p.link} target="_blank" rel="noopener noreferrer" key={i} className="block w-full">
              {content}
            </a>
          );
        })}
